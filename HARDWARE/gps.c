...
{			 
	u16 len=0,i;
	u8 rval=0;
	while((UART4_RX_STA&0X8000)==0 && len<100)//等待接收到应答   
	{
		len++;
		delay_ms(5);
	}		 
	if(len<100)   	//超时错误.
	{
		len=UART4_RX_STA&0X7FFF;	//此次接收到的数据长度 
		for(i=0;i<len;i++)
		{
			if(UART4_RX_BUF[i]==0X83)break;
			else if(UART4_RX_BUF[i]==0X84)
			{
				rval=3;
				break;
			}
...
{
	SkyTra_baudrate *cfg_prt=(SkyTra_baudrate *)UART4_TX_BUF;
	...
	delay_ms(200);				//等待发送完成 
	UART4_Init(BAUD_id[baud_id]);	//重新初始化串口3  
...
{
	u32 temp=width;
	SkyTra_pps_width *cfg_tp=(SkyTra_pps_width *)UART4_TX_BUF;
...
{
	SkyTra_PosRate *cfg_rate=(SkyTra_PosRate *)UART4_TX_BUF;
