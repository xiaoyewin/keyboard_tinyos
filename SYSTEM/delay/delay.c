...
void delay_init()
{
	...
	u32 reload;
#endif
	SysTick_CLKSourceConfig(SysTick_CLKSource_HCLK_Div8);	//选择外部时钟  HCLK/8   为9MHZ     选择滴答寄存器的时钟   FCLK=HCLK=72M
	fac_us=SystemCoreClock/8000000;				//为系统时钟的1/8    9     系统的时钟为72000000     那么  fac_us=9      9下为1us
#if SYSTEM_SUPPORT_OS  							//如果需要支持OS.
...
#else
	fac_ms=(u16)fac_us*1000;					//非OS下,代表每个ms需要的systick时钟数     9000   这是静态变量
#endif
}								    
