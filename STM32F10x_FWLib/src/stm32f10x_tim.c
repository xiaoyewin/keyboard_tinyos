...
void TIM_TimeBaseInit(TIM_TypeDef* TIMx, TIM_TimeBaseInitTypeDef* TIM_TimeBaseInitStruct)
{
  ...
  {
    /* Set the clock division */     //不知道这个时钟分频因子有什么用
  ...
  TIMx->PSC = TIM_TimeBaseInitStruct->TIM_Prescaler;   //计数器的时钟频率CK_CNT等于fCK_PSC/(PSC[15:0]+1)。   来源于pclk1  3600M
  ...
  /* Generate an update event to reload the Prescaler and the Repetition counter
     values immediately */
  TIMx->EGR = TIM_PSCReloadMode_Immediate;       //产生更新事件          
}
