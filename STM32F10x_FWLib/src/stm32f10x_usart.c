...
void USART_Init(USART_TypeDef* USARTx, USART_InitTypeDef* USART_InitStruct)
{
  ...
  if (USART_InitStruct->USART_HardwareFlowControl != USART_HardwareFlowControl_None)    //确定有没有硬件流
  {
    assert_param(IS_USART_123_PERIPH(USARTx));    //只有串口1,2,3能控制硬件流
  }
...
/*---------------------------- USART CR2 Configuration -----------------------*/
  ...
  tmpreg &= CR2_STOP_CLEAR_Mask;    //先清除该2位的信息，确定停止位
...
/*---------------------------- USART BRR Configuration -----------------------*/
  ...
  RCC_GetClocksFreq(&RCC_ClocksStatus);     //得到系统的所有时钟
